...
| Traction, honestly | Small, and measured on chain rather than from our own counter. Over the service's whole life, across both rails, **eight payer addresses that are not ours sent 39 payments totalling 0.360000 USD₮0** to the advertised payTo. A third of a dollar is not a business. This row has now been wrong twice in opposite directions, and both corrections belong here. It first said three wallets and zero recurrence, which came from an in-memory instrument that resets on every deploy, in a document whose argument is that a number you cannot re-derive is a number you should not trust. It then said six addresses, 44 payments and 0.575 USD₮0 over the eight days to 27 July 2026 — and that figure silently included our own field-test wallet, which sent exactly half of those payments. Re-scanning the block range that row itself named, with no gaps, the external half is five addresses and 22 payments totalling 0.215000. **Counting your own buyer as somebody else's is the more embarrassing of the two errors, because it errs upward.** Everything else is our own disclosed quality-assurance traffic, never counted as sales. The buyer audit in Section 6.5 was commissioned by us, so it is arm's length, not independent. |
...
| Proof registry that verifies a Quiver answer's arithmetic on chain, without trusting the seller | `0xd50A91E36673443749Ee22031cb2Ff09d4Bb8D60` verifier 0x59F6Aa860eE0d26Db873f7c7015CE869170b3b25 | X Layer (eip155:196), deployed 28 July 2026 okx.com/web3/explorer/xlayer/… the registry holds the verifier and attestor addresses as immutables; read them back with `verifier()` and `quiverAttestor()` rather than taking this row's word for it |
| A proof **bought from the live endpoint**, accepted on chain | `0x50397d713b96414800fef2dc6c2b4b8a48bd89d7f793683df9deddfbe73f368a` | X Layer (eip155:196), block 66,412,787, 468,459 gas, event `ProofAccepted` okx.com/web3/explorer/xlayer/… the registry stores `58329.113924051` for this position against the `58329.11` the service sold; `liquidationPrice(0x25669da5…)` returns it |
| The same proof with the certified price moved one grid step, **rejected** on chain | `0x97502c78e61958a9a1013a257bf281c665684e214d6474c41444eb0294cb4aac` | X Layer (eip155:196), block 66,412,794, 333,155 gas, event `ProofRejected` okx.com/web3/explorer/xlayer/… one part in 10^9 is enough to invalidate it. A verifier that cannot refuse is not a verifier, so the refusal is published beside the acceptance |
