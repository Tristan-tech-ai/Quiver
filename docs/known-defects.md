...
| of those, with a compiled `.r1cs` **committed** anywhere under `zk/build` | **2 of 38** — `lpclosed` and `lpclosed2`, committed 30 July with `hez_final_13.ptau`, so those two ARE rebuildable from a clone. Eleven more are compiled on this desk and committed nowhere: `xacommit`, `xamin`, `xapriv`, `ncdfonesided`, `pg5`, `pg6`, `pg7`, `pgb4`, `pgb6`, `pgc6`, `pgc7`, `portfoliogate4` — built 22:44 UTC on 30 July, **uncommitted**, and therefore in no clone. Counting them as coverage would be counting a directory listing this repository does not publish. |
| Plonk zkeys for any of them | **0 of 38 committed.** Four exist on this desk and in no clone: `xacommit_plonk.zkey`, `xamin_plonk.zkey`, `xapriv_plonk.zkey` and `lpclosed2_plonk.zkey`, plus `lpclosed_plonk.zkey` in the build root. **30,206,964 bytes** of adversary artifacts under `zk/build/adv`, none of it committed. `lpclosed` and `lpclosed2` are the exception that proves the rule: their `.r1cs` and the `hez_final_13.ptau` they need ARE committed, so a clone regenerates those two zkeys with one `plonk setup` in about 930 ms. Nothing regenerates the other nine. Asked at every depth under `zk/build`, and for `_plonk.zkey`, `.zkey` and `_final.zkey` |
