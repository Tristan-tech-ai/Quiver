...
| …and the closed-form route instead of the quadrature | **also the ceremony file, but only one power up — and now COMPILED rather than projected.** `lpclosed.circom` is **3,854 R1CS → 7,471 Plonk → domain 8,192**, an inflation of 1.9385×. snarkjs refuses the smaller file in as many words: `circuit too big for this power of tau ceremony. 7471 > 2**12`. The earlier 3,023 was `probe-lpclosed-cost.json`'s COST MODEL, not this circuit, and that probe read an R1CS count against a Plonk ceiling to conclude it "fits `hez_final_12` with room to spare"; corrected in the file. `lpclosed2.circom` is the variant that genuinely fits 2^12 at 1,847 R1CS / 3,554 Plonk, paying for it with a power-of-two scale and a v cap of 256. **`hez_final_13` is a public download and moves no hash**; it is committed, and gate `zk/scripts/gateLPC-closed-form.mjs` asserts the 2^12 refusal rather than restating it. |
