...
An attestation of this root under the `risk-attest` EAS schema on Base (`0x59a8587b287d3f13776dccbe49e19d2e887f90b5e16650464b07e613d89287e0`, schema string `bytes32 merkleRoot, uint256 itemCount, string engineVersion`) is prepared but **not yet submitted**, and the reason is narrower than "we cannot": the key that would sign it is not the one available here.

Two different wallets are involved and it is worth separating them. The disposable payer used for x402 settlement, `0x15C44cD124773A43B998EC0417b1a20B52cF5AA6`, holds no ETH on Base at all, because the x402 path never needs gas — the facilitator pays it. The operator wallet that owns the on-chain agent identity, `0x65bb932D9987F1d1a98b8942a3fa98CB28Ec073B`, *has* anchored a batch root under this schema before: attestation `0x69d42632f17d9286364dd23a94de3453f1cef76b01deca21173e007f9cab90c2` on Base, 20 July 2026, itemCount 2. So the capability is demonstrated and the schema works; what is missing is a submission of *this particular* root.

It has not been made because using the owner key requires an environment this repository is not developed in, and the operational separation between that key and the development machine is deliberate and is not being relaxed days before a submission deadline to improve a document. That is a scheduling and key-hygiene decision, not a technical limit, and it should be read as one: a reader is entitled to note that the strongest form of this guarantee was available to us and we did not take it in time. When it is submitted the attestation UID will be added here and to Appendix C of the paper. Until then, treat the guarantee above as "unchanged relative to a git-timestamped manifest", which is weaker than an on-chain anchor and stronger than nothing.
