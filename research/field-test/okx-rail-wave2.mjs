// OKX-rail field test — WAVE 2: the five higher-priced services, buyer Account 2 (#6166), TEE-signed.
...
fs.writeFileSync(new URL('./okx-rail-wave2-results.json', import.meta.url), JSON.stringify({ atUtc: new Date().toISOString(), rows, totalSpentUsd: Math.round(spent * 1000) / 1000 }, null, 1));
console.log('\nWAVE 2 TOTAL:', '$' + spent.toFixed(3), '| 200s:', rows.filter((r) => r.http === 200).length + '/' + rows.length, '| hashOk:', rows.filter((r) => r.hashOk).length);
