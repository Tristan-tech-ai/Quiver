...
-- Feb-2026 event. Change the file names + threshold for Feb-2026 (see ablation-feb.sql).
...
SELECT COUNT(*) FILTER (WHERE downKillBeta<=21.88) AS n_flagged, COUNT(*) AS n_total FROM flags;
SELECT * FROM sizes;
...
SELECT 'A_beta_scaled' AS arm,
  SUM(CASE WHEN downKillBeta<=21.88 THEN 1 ELSE 0 END) AS flagged,
  ROUND(100.0*SUM(CASE WHEN downKillBeta<=21.88 AND wiped THEN 1 ELSE 0 END)/NULLIF(SUM(CASE WHEN downKillBeta<=21.88 THEN 1 ELSE 0 END),0),2) AS pct_flagged_wiped,
  ROUND(100.0*SUM(CASE WHEN downKillBeta>21.88 AND wiped THEN 1 ELSE 0 END)/NULLIF(SUM(CASE WHEN downKillBeta>21.88 THEN 1 ELSE 0 END),0),2) AS pct_cleared_wiped,
  ROUND((1.0*SUM(CASE WHEN downKillBeta<=21.88 AND wiped THEN 1 ELSE 0 END)/NULLIF(SUM(CASE WHEN downKillBeta<=21.88 THEN 1 ELSE 0 END),0))
      / NULLIF(1.0*SUM(CASE WHEN downKillBeta>21.88 AND wiped THEN 1 ELSE 0 END)/NULLIF(SUM(CASE WHEN downKillBeta>21.88 THEN 1 ELSE 0 END),0),0),2) AS relative_risk
FROM flags
UNION ALL
