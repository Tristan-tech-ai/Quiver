...
contract PlonkVerifier {
    ...
    uint256 constant Qmx  = 17551248126354335136096889127929232817023171226054449466743550621958200480115;
    uint256 constant Qmy  = 17820000058139051217707159197841880586854349897074914693338167817157375995678;
    uint256 constant Qlx  = 6491374800087969375136898325342968690295150697565076138473318738222226966164;
    uint256 constant Qly  = 12445681060283270512484566212056073761312319390233821523534065538503386625817;
    uint256 constant Qrx  = 11756009403433339397899091748705620304292102296436444537533231729094417703611;
    uint256 constant Qry  = 14715102804373159474097215510600558852826624891866798906775785406558675615491;
    uint256 constant Qox  = 5881435382833630708361030894925359316752882463649217105196636973331313083304;
    uint256 constant Qoy  = 12032700760673679408713126237989902348741106260828407643383945603199480929894;
    uint256 constant Qcx  = 1412919420185408649623796489121368713614443139886853237116366879584972457665;
    uint256 constant Qcy  = 7685510135469907955116827035994870565692323090902932293970177738262590408166;
    uint256 constant S1x  = 3139947018781877108223429590641919544340203379549087498272795733953247996257;
    uint256 constant S1y  = 5500544554418988775440093267033125059940682579555924773196228608112985118317;
    uint256 constant S2x  = 17983316526562810312521646984926895125529377956352930461675260599126853436830;
    uint256 constant S2y  = 14319078313770002255149043073708220726945814942820266855893756614381816648079;
    uint256 constant S3x  = 17898205002139888484266216149206080407429832591032292023704279127824015220409;
    uint256 constant S3y  = 21446978821464356577443814129402618671344255703844953309692959988205752332916;
    ...
    uint16 constant lastMem = 1728;

    function verifyProof(uint256[24] calldata _proof, uint256[29] calldata _pubSignals) public view returns (bool) {
        assembly {
            /////////
            ...
            function calculateChallenges(pMem, pPublic) {
                ...
                mstore(add(mIn, 1440 ), calldataload(pA))
                mstore(add(mIn, 1472 ), calldataload(add(pA, 32)))
                mstore(add(mIn, 1504 ), calldataload(pB))
                mstore(add(mIn, 1536 ), calldataload(add(pB, 32)))
                mstore(add(mIn, 1568 ), calldataload(pC))
                mstore(add(mIn, 1600 ), calldataload(add(pC, 32)))
                
                beta := mod(keccak256(mIn, 1632), q) 
