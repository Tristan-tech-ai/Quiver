...
contract PlonkVerifier {
    ...
    uint16 constant nPublic   = 28;
    uint16 constant nLagrange = 28;
    
    uint256 constant Qmx  = 21098042717086358859127205740083848365674655072460536031604074146063358151876;
    uint256 constant Qmy  = 4411086416334091862579204733504673982355858378856164694555370931255335271822;
    uint256 constant Qlx  = 8379805720085178788938160319278376018031400871442743143947123537882466308199;
    uint256 constant Qly  = 14777059124546842590734822060211355205928223343386893245705468029434406881314;
    uint256 constant Qrx  = 8252998953438311111729217766890467197746396382834623658915400986096433424435;
    uint256 constant Qry  = 2127809753819209023889938989231739969249307447603474073976727931849200028372;
    uint256 constant Qox  = 3753741756300619940970914705010920120335486833755315785361464913771591696772;
    uint256 constant Qoy  = 10611822511632952030022126227539482803653570397026391221327768451529386400808;
    uint256 constant Qcx  = 1409508932636125438992709041751857809217779928186463375135051868371960840932;
    uint256 constant Qcy  = 10688255109170940270208655474924386013124067380259418402874310881337465235914;
    uint256 constant S1x  = 17024017293557124472754168914097959272188474098351173586982689903974736157476;
    uint256 constant S1y  = 2129200298521318078796532029008646931313582234174329794475779420240575687199;
    uint256 constant S2x  = 21251499877766019541030975944315113450012858666900321138462543590810338152050;
    uint256 constant S2y  = 3151917709428825067348061604933356349122131729891603307140822836566918109137;
    uint256 constant S3x  = 7281842797484987274003370212041356190108160089621962395833696185899593272340;
    uint256 constant S3y  = 6859622401304443758051447595226935330400724131018181709330442539470445574099;
    ...
    uint16 constant lastMem = 1696;

    function verifyProof(uint256[24] calldata _proof, uint256[28] calldata _pubSignals) public view returns (bool) {
        assembly {
            /////////
            ...
            function calculateChallenges(pMem, pPublic) {
                ...
                mstore(add(mIn, 1408 ), calldataload(pA))
                mstore(add(mIn, 1440 ), calldataload(add(pA, 32)))
                mstore(add(mIn, 1472 ), calldataload(pB))
                mstore(add(mIn, 1504 ), calldataload(add(pB, 32)))
                mstore(add(mIn, 1536 ), calldataload(pC))
                mstore(add(mIn, 1568 ), calldataload(add(pC, 32)))
                
                beta := mod(keccak256(mIn, 1600), q) 
            ...
            function calculateLagrange(pMem) {
                ...
                inverseArray(add(pMem, pZhInv), 29 )
