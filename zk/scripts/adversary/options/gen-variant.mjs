...
function emit(C, P) {
  ...
  w('');
  // The include depth is a function of WHERE the circuit is written, not of the mathematics. It is a
  // parameter so that regenerating a committed circuit is a byte-for-byte comparison rather than a
  // comparison that has to forgive three lines — a diff that forgives lines is a diff that can be
  // made to forgive the wrong ones. Default keeps the shipped generator's `circuits/` depth.
  const INC = process.env.VINC || '../node_modules/circomlib/circuits/';
  w(`include "${INC}bitify.circom";`);
  w(`include "${INC}comparators.circom";`);
  w(`include "${INC}mux4.circom";`);
  w('');
...
writeFileSync(path.join(ZK, 'circuits', (process.env.VNAME||'ncdf')+'.circom'), src, 'utf8');
writeFileSync(path.join(ZK, 'build', (process.env.VNAME||'ncdf')+'-consts.json'), JSON.stringify({
  params: P,
