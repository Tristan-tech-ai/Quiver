...
const LIQUIDATION_ONE_LEG = 273_118n; // measured: one Plonk verify, 8 public signals
...
console.log(`  one liquidation proof, 8 public signals     ${String(LIQUIDATION_ONE_LEG).padStart(10)}   (gateB6)`);
...
record('one wide proof verifies for less than three separate ones would',
  ...
  `${evm.acceptGas} against ${LIQUIDATION_ONE_LEG * 3n} for three single-leg verifies`);
